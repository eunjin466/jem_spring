...
public class FileService {
	...
	public List<String> uploadFile(List<MultipartFile> multipartFiles) throws IOException {
		...
		for(MultipartFile multipartFile : multipartFiles) {
			String fileurls =uploadFile(multipartFile);
			fileUrls.add(fileurls);
		}
		return fileUrls;
	}
	private String fileSave(String originalName, byte[] fileData) throws IOException { // 진짜 없로드 해주는 메서드
		
		UUID uuid = UUID.randomUUID(); //이름 중복을 방지하기 위한 랜덤 문자열 발생
